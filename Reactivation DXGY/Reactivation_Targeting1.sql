...
SELECT 
    Z.driver_id AS "Driver ID", 
    Z.driver_uuid AS "Driver UUID", 
    cur_zone AS "Current Zone", 
    current_oa AS "Current Onboarding Area", 
    most_common_oa AS "Most Common OA", 
    O.last_work_date AS "Last Work Date", 
    total_orders AS "Total Orders", 
    last_4w AS "Orders Last 4W", 
    last_28w_avg AS "Orders 28W Avg"
...
SELECT DAD.driver_id, DAD.driver_uuid, DAD.status, B.braze_email_subscribe 

FROM production.denormalised.driver_accounting_daily AS DAD 
LEFT JOIN Braze AS B 
    ON DAD.driver_uuid = B.rider_uuid

WHERE DAD.DRIVER_ID IN
        ()
 ...
 SELECT 
    ...
    select * FROM production.braze.rider_stats_daily_log limit 20;
    select * FROM production.braze.braze_riders order by user_id desc limit 200;