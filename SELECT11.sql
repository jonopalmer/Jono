SELECT
    ...
    ROUND((COALESCE(SUM(rider_hours_worked_dhw_sum_excl_ghost_and_oos_hours), 0)
    - (SUM(potential_orders_from_hidden_rx) + SUM(orders_delivered)) * SUM(r_star_raw_0day) / NULLIF(SUM(v_hat_raw_0day), 0) )
    / (SUM(potential_orders_from_hidden_rx) + SUM(orders_delivered)) * SUM(r_star_raw_0day) / NULLIF(SUM(v_hat_raw_0day), 0),3) AS "R vs Retro R",
...
WHERE START_OF_PERIOD_LOCAL >= DATEADD('week', -5, DATE_TRUNC('week', DATE_TRUNC('day', CONVERT_TIMEZONE('UTC', 'Europe/London', CURRENT_TIMESTAMP()))))
    ...
    AND orders_delivered > 10
    AND CITY_NAME IN ('Southampton', 'London', 'Fife & Perthshire', 'Sevenoaks', 'Maidstone', 'Camberley', 'Woking')
    

GROUP BY city_name, cluster_name
HAVING utilisation_adjusted is not null
ORDER BY avg_throughput DESC NULLS LAST, cluster_name DESC NULLS LAST
