...
GROUP BY date, zone_code, peak
ORDER BY zone_code, date DESC, peak DESC
),
CTE2 AS(
SELECT
    ...
    AVG(ROUND(NULLIF(SUM(orders_delivered), 0) / NULLIF(SUM(rider_hours_worked_dhw_sum_excl_ghost_and_oos_hours), 0),3)) OVER (PARTITION BY zone_code) AS avg_throughput
...
GROUP BY date, day, zone_code, peak    
ORDER BY avg_throughput DESC, zone_code DESC, date, peak
)
...
FROM CTE2 a
LEFT JOIN CTE b ON a.zone_code = b.zone_code AND a.date = b.date AND a.peak = b.peak
ORDER BY a.zone_code, a.date DESC, a.peak DESC
