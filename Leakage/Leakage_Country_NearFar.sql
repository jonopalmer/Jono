...
CREATE OR REPLACE TEMPORARY TABLE FT_cities_worked_in AS (
    ...
    FROM production.denormalised.driver_hours_worked AS a
        ...
        LEFT JOIN CITY_DISTANCES AS d ON a.city || b.city_name = d.CITY_COMBO
...
--TEMP
SELECT
    B.city_detailed AS CITY, A.COUNTRY_NAME
...
GROUP BY B.city_detailed, A.COUNTRY_NAME
ORDER BY A.COUNTRY_NAME, B.city_detailed
;
...
SELECT A.CITY || B.CITY AS CITY_COMBO,
    ...
    CASE WHEN DISTANCE_KM > 15 OR DISTANCE_KM IS NULL THEN 'FAR'
        ELSE 'NEAR'
...
SELECT * FROM CITY_DISTANCES WHERE CITY_1 = 'London' order by DISTANCE_KM
;
...
SELECT * FROM CITY_DISTANCES_CITY_NAME WHERE CITY_1 = 'London' ORDER BY DISTANCE_KM
;
...
SELECT
    ZONE_CODE,
    CITY_NAME AS CITY,
    CENTROID_GEO_LAT AS GEO_LAT, 
    CENTROID_GEO_LONG AS GEO_LONG
FROM PRODUCTION.REFERENCE.ZONE_CITY_COUNTRY
WHERE COUNTRY_NAME IN ('France', 'Italy', 'Belgium', 'UK', 'Ireland')
...
(
    ...
    ROUND(COUNT(CASE WHEN a.DISTANCE <> 'NEAR' THEN 1 END) / COUNT(DISTINCT a.driver_id), 3) AS "% Riders Never Worked NEAR OB City",
    COUNT(CASE WHEN a.DISTANCE <> 'NEAR' THEN 1 END) AS "Riders Never Worked NEAR OB City",
